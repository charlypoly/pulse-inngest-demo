...
class InngestCommHandler {
    constructor(options) {
        ...
        this._serveHost = options.serveHost || this.env[consts_1.envKeys.InngestServeHost];
        this._servePath = options.servePath || this.env[consts_1.envKeys.InngestServePath];
        ...
        this.fetch = options.fetch ? (0, env_1.getFetch)(options.fetch) : this.client["fetch"];
    }
    /**
    ...
    get eventApiBaseUrl() {
        return (this._options.baseUrl ||
            this.env[consts_1.envKeys.InngestEventApiBaseUrl] ||
            this.env[consts_1.envKeys.InngestBaseUrl] ||
            this.client.eventBaseUrl ||
            consts_1.defaultInngestEventBaseUrl);
    }
    /**
     ...
     */
    get serveHost() {
        return this._serveHost || this.env[consts_1.envKeys.InngestServeHost];
    }
    /**
     ...
     */
    get servePath() {
        return this._servePath || this.env[consts_1.envKeys.InngestServePath];
    }
    get hashedEventKey() {
        if (!this.client["eventKey"] || this.client["eventKey"] === consts_1.dummyEventKey) {
            return undefined;
        }
        return (0, strings_1.hashEventKey)(this.client["eventKey"]);
    }
    ...
    createHandler() {
        const handler = async (...args) => {
            ...
            const headerPromises = [
                consts_1.headerKeys.TraceParent,
                consts_1.headerKeys.TraceState,
            ].map(async (header) => {
            ...
            const [signature, method, body] = await Promise.all([
                actions
                    .headers("checking signature for request", consts_1.headerKeys.Signature)
            ...
            const prepareActionRes = async (res) => {
                ...
                return Object.assign(Object.assign({}, res), { headers: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, getInngestHeaders()), (await headersToForwardP)), res.headers), (res.version === null
                        ? {}
                        : {
                        ...
                        })), (signature ? { [consts_1.headerKeys.Signature]: signature } : {})) });
            };
            ...
            if (method === "POST") {
                ...
                if (!validationResult.success) {
                    return {
                        ...
                        body: (0, strings_1.stringify)((0, errors_1.serializeError)(validationResult.err)),
                        version: undefined,
                    };
                }
                const rawProbe = await getQuerystring("testing for probe", consts_1.queryKeys.Probe);
                if (rawProbe) {
                    const probe = (0, enum_1.enumFromValue)(consts_1.probe, rawProbe);
                    if (!probe) {
                        ...
                        return {
                            ...
                            body: (0, strings_1.stringify)((0, errors_1.serializeError)(new Error(`Unknown probe "${rawProbe}"`))),
                            version: undefined,
                        };
                    }
                    // Provide actions for every probe available.
                    const probeActions = {
                        [consts_1.probe.Trust]: () => ({
                            status: 200,
                            headers: {
                                "Content-Type": "application/json",
                            },
            ...
            if (method === "GET") {
                ...
                if (this._mode.type === "cloud") {
                    try {
                        ...
                        introspection = Object.assign(Object.assign({}, introspection), { authentication_succeeded: true, api_origin: this.apiBaseUrl, app_id: this.client.id, capabilities: {
                                trust_probe: "v1",
                            }, env: (await actions.headers("fetching environment for introspection request", consts_1.headerKeys.Environment)) || null, event_api_origin: this.eventApiBaseUrl, event_key_hash: (_a = this.hashedEventKey) !== null && _a !== void 0 ? _a : null, extra: Object.assign(Object.assign({}, introspection.extra), { is_streaming: this.shouldStream(actions) }), framework: this.frameworkName, sdk_language: "js", sdk_version: version_1.version, serve_origin: (_b = this.serveHost) !== null && _b !== void 0 ? _b : null, serve_path: (_c = this.servePath) !== null && _c !== void 0 ? _c : null, signing_key_fallback_hash: (_d = this.hashedSigningKeyFallback) !== null && _d !== void 0 ? _d : null, signing_key_hash: (_e = this.hashedSigningKey) !== null && _e !== void 0 ? _e : null });
                    }
                    catch (_f) {
    ...
    configs(url) {
        const configs = Object.values(this.rawFns).reduce((acc, fn) => [...acc, ...fn["getConfig"](url, this.id)], []);
        for (const config of configs) {
            const check = types_1.functionConfigSchema.safeParse(config);
            if (!check.success) {
    ...
    async register(url, deployId, getHeaders) {
        ...
        if (inferredDevMode) {
            ...
            registerURL = (0, devserver_1.devServerUrl)(this._mode.explicitDevUrl.href, "/fn/register");
        }
        if (deployId) {
    ...
    getResponseSignature(key, body) {
        const now = Date.now();
        const mac = (0, net_1.signDataWithKey)(body, key, now.toString());
        return `t=${now}&s=${mac}`;
    }
    /**
...
_RequestSignature_instances = new WeakSet(), _RequestSignature_verifySignature = function _RequestSignature_verifySignature({ body, signingKey, allowExpiredSignatures, }) {
    ...
    const mac = (0, net_1.signDataWithKey)(body, signingKey, this.timestamp);
