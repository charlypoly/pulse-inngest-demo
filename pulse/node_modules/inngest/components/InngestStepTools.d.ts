...
import { type ExclusiveKeys, type ParametersExceptFirst, type SendEventPayload, type SimplifyDeep, type WithoutInternalStr } from "../helpers/types";
import { type EventPayload, type HashedOp, type InvocationResult, type InvokeTargetFunctionDefinition, type SendEventOutput, type StepOptions, type StepOptionsOrId, type TriggerEventFromFunction, type TriggersFromClient } from "../types";
import { type ClientOptionsFromInngest, type GetEvents, type GetFunctionOutput, type Inngest } from "./Inngest";
...
export declare const createStepTools: <TClient extends Inngest.Any>(client: TClient, execution: InngestExecution, stepHandler: StepHandler) => {
     ...
     * for next steps.
     */
    run: <T extends () => unknown>(idOrOptions: StepOptionsOrId, fn: T) => Promise<
    /**
     * TODO Middleware can affect this. If run input middleware has returned
     * new step data, do not Jsonify.
     */
    SimplifyDeep<Jsonify<T extends () => Promise<infer U> ? Awaited<U extends void ? null : U> : ReturnType<T> extends void ? null : ReturnType<T>>>>;
    /**
